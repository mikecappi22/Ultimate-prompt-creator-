/* Ultimate Prompt Creator V13.1 Subject Vault — text metadata + local reference gallery only */
(function(global){
'use strict';
if(global.__UPC_SUBJECT_VAULT_V131__)return;global.__UPC_SUBJECT_VAULT_V131__=true;
...
function loadSubjects(){
  try{const raw=localStorage.getItem(STORAGE_KEY);const a=raw?JSON.parse(raw):null;if(Array.isArray(a)&&a.length)return a}catch(_){ }
  const a=[seedAddison()];saveSubjects(a);return a
}
...
function buildLock(s,mode='full'){
  if(!s)return'';
  const lines=[`[SUBJECT LOCK — ${String(s.name||'SUBJECT').trim().toUpperCase()}]`,fieldLine('SUBJECT',`${String(s.name||'').trim()}${s.type?` — ${String(s.type).trim()}`:''}`)];
  const traits=[['FACE',s.face],['EYES',s.eyes],['HAIR',s.hair],['SKIN',s.skin],['BODY / PROPORTIONS',s.body],['DISTINGUISHING FEATURES',s.distinguishing],['REFERENCE NOTES',s.referenceNotes]];
  const defined=traits.filter(([,v])=>String(v||'').trim());
  if(defined.length)for(const [k,v] of defined)lines.push(fieldLine(k,v));
...
function mount(){
  ...
  <div class="sv-grid"><div><label>Display name</label><input id="svName"></div><div><label>Subject type</label><input id="svType" value="Adult woman"></div></div>
  ...
  <label>Reference notes</label><textarea id="svReferenceNotes" rows="3" placeholder="Any manual notes about reference photos or recurring styling."></textarea>
  ...
  <div class="sv-section"><div class="sv-head"><div><b>Local reference photos</b><div class="sv-note">Stored only in this browser using IndexedDB. They are not analyzed, sent to Ollama, or uploaded to GitHub.</div></div><span id="svPhotoCount" class="sv-note"></span></div><input id="svPhotoInput" class="sv-photo-input" type="file" accept="image/jpeg,image/png,image/webp" multiple><div id="svPhotos" class="sv-photos"></div></div>
...
async function deleteSubject(){const s=active();if(!s)return;if(!confirm(`Delete ${s.name}? Text profile will be removed. Local reference photos for this subject will also be deleted.`))return;await deleteSubjectPhotos(s.id);subjects=subjects.filter(x=>x.id!==s.id);if(!subjects.length)subjects=[seedAddison()];activeId=subjects[0].id;persist();renderAll();status('Subject deleted.','good')}
...
function openPhotoDb(){if(photoDbPromise)return photoDbPromise;photoDbPromise=new Promise((resolve,reject)=>{if(!('indexedDB'in global)){reject(new Error('IndexedDB unavailable'));return}const r=indexedDB.open(PHOTO_DB,1);r.onupgradeneeded=()=>{const db=r.result;if(!db.objectStoreNames.contains(PHOTO_STORE)){const st=db.createObjectStore(PHOTO_STORE,{keyPath:'id'});st.createIndex('subjectId','subjectId',{unique:false})}};r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error||new Error('Could not open photo storage'))});return photoDbPromise}
async function photoList(subjectId){const db=await openPhotoDb();return new Promise((resolve,reject)=>{const tx=db.transaction(PHOTO_STORE,'readonly'),idx=tx.objectStore(PHOTO_STORE).index('subjectId'),r=idx.getAll(subjectId);r.onsuccess=()=>resolve(r.result||[]);r.onerror=()=>reject(r.error)})}
async function putPhoto(rec){const db=await openPhotoDb();return new Promise((resolve,reject)=>{const tx=db.transaction(PHOTO_STORE,'readwrite');tx.objectStore(PHOTO_STORE).put(rec);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error)})}
async function removePhoto(id){const db=await openPhotoDb();return new Promise((resolve,reject)=>{const tx=db.transaction(PHOTO_STORE,'readwrite');tx.objectStore(PHOTO_STORE).delete(id);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error)})}
...
async function compressPhoto(file){const src=await fileToDataURL(file),img=await loadImg(src),max=900,scale=Math.min(1,max/Math.max(img.naturalWidth||img.width,img.naturalHeight||img.height)),w=Math.max(1,Math.round((img.naturalWidth||img.width)*scale)),h=Math.max(1,Math.round((img.naturalHeight||img.height)*scale)),cv=document.createElement('canvas');cv.width=w;cv.height=h;const ctx=cv.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,w,h);ctx.drawImage(img,0,0,w,h);return {dataUrl:cv.toDataURL('image/jpeg',.80),width:w,height:h}}
async function addPhotos(files){const s=active();if(!s)return;try{let list=await photoList(s.id),room=MAX_PHOTOS-list.length;if(room<=0){status(`${s.name} already has ${MAX_PHOTOS} local reference photos.`,'bad');return}for(const file of [...files].slice(0,room)){const c=await compressPhoto(file);await putPhoto({id:`photo-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,7)}`,subjectId:s.id,name:file.name||'reference.jpg',dataUrl:c.dataUrl,width:c.width,height:c.height,createdAt:new Date().toISOString()})}await renderPhotos();status('Reference photo saved locally. No image analysis was performed.','good')}catch(e){status('Could not save reference photo: '+e.message,'bad')}}
async function renderPhotos(){const w=$('svPhotos'),c=$('svPhotoCount');if(!w)return;w.innerHTML='';try{const list=await photoList(active()?.id);if(c)c.textContent=`${list.length}/${MAX_PHOTOS} saved locally`;for(const p of list){const box=document.createElement('div');box.className='sv-photo';const img=document.createElement('img');img.src=p.dataUrl;img.alt=p.name||'Reference';img.title=p.name||'Reference';const b=document.createElement('button');b.type='button';b.textContent='×';b.title='Remove reference';b.onclick=async()=>{await removePhoto(p.id);await renderPhotos();status('Reference photo removed.','good')};box.append(img,b);w.appendChild(box)}if(!list.length)w.innerHTML='<div class="sv-note">No local reference photos yet.</div>'}catch(e){if(c)c.textContent='Local photo storage unavailable';w.innerHTML='<div class="sv-note">Reference gallery unavailable in this browser.</div>'}}
function renderAll(){renderStrip();renderEditor();renderPhotos()}
function bind(){
  $('svSave').onclick=()=>saveCurrent(true);$('svNew').onclick=newSubject;$('svDelete').onclick=deleteSubject;$('svCopy').onclick=()=>copyText($('svLockPreview').value);$('svApply').onclick=applyToPrompt;$('svDirector').onclick=sendDirector;$('svExport').onclick=exportSubjects;$('svImportBtn').onclick=()=>$('svImport').click();$('svImport').onchange=e=>{const f=e.target.files?.[0];if(f)importSubjects(f);e.target.value=''};$('svPhotoInput').onchange=e=>{if(e.target.files?.length)addPhotos(e.target.files);e.target.value=''};$('svLockMode').onchange=updateLock;for(const id of Object.keys(fieldMap))$(id)?.addEventListener('input',updateLock)
}
