...
$Ref='62953a72db7bb93e7b644b0b6d19470bec180026'
$Dir=Join-Path $env:LOCALAPPDATA 'UltimatePromptCreatorMobile'
$App=Join-Path $Dir 'index.html'
$Backup=Join-Path $Dir ('index-before-v145-'+(Get-Date -Format 'yyyyMMdd-HHmmss')+'.html')
$Bridge=Join-Path $Dir 'private-mobile-proxy-v1187.ps1'
$Tmp=Join-Path $env:TEMP 'UPC-V145'
$PrivateUrl='https://laptop-5efmmkr5.tail97be36.ts.net/'
$Utf8NoBom=New-Object System.Text.UTF8Encoding($false)

Write-Host '=== Ultimate Prompt Creator V14.5 Installer ===' -ForegroundColor Cyan
Write-Host 'Projects + Prompt History + Shot Builder + Smart Build + Subject Vault + Prompt Library.'
...
Write-Host 'Step 1/8 - Downloading pinned V14.5 components...'
...
Write-Host 'Step 2/8 - Assembling V14.5 Production Studio...'
...
$productionText=Read-Utf8 $Production
$featureScripts="<script>`r`n$composerText`r`n</script>`r`n<script>`r`n$productionText`r`n</script>`r`n</body>"
$html=$html.Replace('</body>',$featureScripts)
$html=$html.Replace('Ultimate Prompt Creator V14.3 Unified','Ultimate Prompt Creator V14.5 Production Studio')
$html=$html.Replace('V14.3 UNIFIED','V14.5 PRODUCTION STUDIO')
$html=$html.Replace('Subjects, Creative Director, Prompt Library and Prompt Workspace in one private app.','Projects, Subjects, Smart Build, Creative Director, Prompt Library and Prompt Workspace in one private app.')
...
foreach($m in @(
  'V14.5 PRODUCTION STUDIO','Subject Vault','Browse / Search Subjects',
  'Smart Prompt Composer','Wardrobe Builder','Prompt Inspector',
  'Production Studio','Projects','Shot Builder','Prompt History',
  'REFERENCE ASSIGNMENTS','CHARACTER COUNT LOCK',
  'Creative Director','Prompt Library','Recommend Suggestions','Prompt Workspace',
  'ADDISON','ANNA','ASHLEY','AVA','BECKIE','Paste Profile JSON'
)){
  if(-not $html.Contains($m)){throw "Candidate page missing required marker: $m"}
}
foreach($bad in @('Export Photo Backup','Import Photo Backup','svPhotoInput','IndexedDB','PHOTO_DB','PHOTO_STORE','Tailscale Funnel')){
  if($html.Contains($bad)){throw "Disallowed code or copy present: $bad"}
}
if($html.Contains('sv-subject-strip')){throw 'Old horizontal subject strip is still present.'}
$candidate=Join-Path $Tmp 'index-v145-candidate.html'
...
foreach($name in @(
  'db-worker-v100.js','db-v8-mobile.v93.json.gz','db-v8-mobile.json',
  'camera-addon-v81.part1.txt','camera-addon-v81.part2.txt','camera-addon-v81.part3.txt',
  'camera-addon-v81.part4.txt','camera-addon-v81.part5.txt'
)){
  if(-not (Test-Path (Join-Path $Dir $name))){throw "Static asset missing after install: $name"}
}
...
Write-Host 'Step 6/8 - Installing V14.5 and restarting private bridge...'
...
try{
  ...
  foreach($m in @(
    'V14.5 PRODUCTION STUDIO','Browse / Search Subjects',
    'Smart Prompt Composer','Wardrobe Builder','Prompt Inspector',
    'Production Studio','Shot Builder','Prompt History',
    'REFERENCE ASSIGNMENTS','CHARACTER COUNT LOCK',
    'Prompt Library','Recommend Suggestions','Prompt Workspace'
  )){
    if(-not $page.Content.Contains($m)){throw "Served page missing $m"}
  }
  foreach($bad in @('Export Photo Backup','Import Photo Backup','svPhotoInput','PHOTO_DB','sv-subject-strip')){
    if($page.Content.Contains($bad)){throw "Served page contains disallowed legacy code: $bad"}
  }
...
try{
  $remote=Invoke-WebRequest ($PrivateUrl+'?v=1450') -UseBasicParsing -TimeoutSec 25
  if($remote.StatusCode -ne 200 -or -not $remote.Content.Contains('V14.5 PRODUCTION STUDIO')){throw 'Private HTTPS page did not return V14.5.'}
  Write-Host 'Private HTTPS PASS.' -ForegroundColor Green
}catch{
  Write-Warning ('Local V14.5 passed but private HTTPS check failed: '+$_.Exception.Message)
  Write-Warning 'Tailscale Serve may need a few seconds; reload the private URL once.'
}

Write-Host ''
Write-Host 'V14.5 PRODUCTION STUDIO IS READY' -ForegroundColor Green
Write-Host $PrivateUrl -ForegroundColor Cyan
Write-Host 'Projects: save/load Subject + Smart Build + Prompt Workspace + Shot Builder as one creative job.'
Write-Host 'Shot Builder: multi-shot director structure with reference, environment, format, character-count, continuity, camera and audio locks.'
Write-Host 'History: local text-only prompt snapshots, restore, search and A/B comparison.'
Write-Host 'Existing V14.4: Smart Composer, Wardrobe Builder, Prompt Recipes and Prompt Inspector remain available.'
Write-Host 'Privacy: Subject Vault and project data remain text-only; reference photos stay external; Ollama remains 127.0.0.1 behind tailnet-only Tailscale Serve.'
Start-Process ($PrivateUrl+'?v=1450')
