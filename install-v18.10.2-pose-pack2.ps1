...
$url = 'https://raw.githubusercontent.com/mikecappi22/Ultimate-prompt-creator-/main/v18-category-pose-pack2-v18102.js'
...
if ($html.Contains('__UPC_V18102_POSE_PACK2__')) {
    Write-Host 'V18.10.2 Pose Add-On Pack 2 is already installed.' -ForegroundColor Yellow
    exit 0
}
...
$backup = Join-Path $dir ('index-before-v18.10.2-pose-pack2-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.html')
Copy-Item $app $backup -Force

Write-Host 'Downloading V18.10.2 Pose Add-On Pack 2...' -ForegroundColor Cyan
$module = (Invoke-WebRequest $url -UseBasicParsing).Content

if (-not $module.Contains('__UPC_V18102_POSE_PACK2__')) {
    throw 'Downloaded pose pack 2 failed marker verification.'
}
...
if (-not $verify.Contains('__UPC_V18102_POSE_PACK2__')) {
...
Write-Host 'PASS  V18.10.2 Pose Add-On Pack 2 installed' -ForegroundColor Green
Write-Host "Backup: $backup" -ForegroundColor DarkGray
Write-Host ''
Write-Host 'Open the app with ?v=18102pose and search Pose for:' -ForegroundColor Cyan
Write-Host '  floor'
Write-Host '  lounging'
Write-Host '  bent'
Write-Host '  yoga'
