...
Write-Host '=== UPC ALWAYS-ON UPDATE TO V11.8.8 ===' -ForegroundColor Cyan
...
if($wt){
  ...
  if($changed){Set-ScheduledTask -TaskName $WatchTask -Action $newActions | Out-Null;Write-Host 'Updated Watchdog scheduled task action.' -ForegroundColor Green}else{Write-Host 'Watchdog task action did not reference v1187; checking watchdog script...' -ForegroundColor Yellow}
}
$watchCandidates=Get-ChildItem $Dir -Filter '*watch*.ps1' -File -ErrorAction SilentlyContinue
foreach($f in $watchCandidates){$txt=[IO.File]::ReadAllText($f.FullName);if($txt -match 'private-mobile-proxy-v1187\.ps1'){$bak=$f.FullName+'.before-v1188.bak';Copy-Item $f.FullName $bak -Force;$txt=$txt -replace 'private-mobile-proxy-v1187\.ps1','private-mobile-proxy-v1188.ps1';[IO.File]::WriteAllText($f.FullName,$txt,(New-Object Text.UTF8Encoding($false)));Write-Host ("Updated watchdog script: "+$f.Name) -ForegroundColor Green}}
Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'private-mobile-proxy-v1187\.ps1' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }
Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'private-mobile-proxy-v1188\.ps1' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }
...
Get-ScheduledTask -TaskName $BridgeTask,$WatchTask -ErrorAction SilentlyContinue | ForEach-Object { $_.TaskName; $_.Actions | Format-List Execute,Arguments }
Write-Host ''
