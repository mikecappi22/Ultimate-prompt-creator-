/* Gainwright Visual Studio V19.0.3 - Core Runtime Test Suite */
(function(g){
'use strict';
if(g.__GAINWRIGHT_CORE_TESTS_V1903__)return;
g.__GAINWRIGHT_CORE_TESTS_V1903__=1;
...
function report(){
 ...
 return ['GAINWRIGHT VISUAL STUDIO V19.0.3 CORE TEST REPORT','Run: '+new Date().toLocaleString(),'Passed: '+p+'/'+results.length+' ('+(results.length?Math.round(p/results.length*100):0)+'%)',''].concat(results.map(function(r){return '['+(r.pass?'PASS':'FAIL')+'] '+r.group+' — '+r.name+(r.msg?' — '+r.msg:'')})).join('\n');
}
function copyReport(){
...
async function runAll(){
 ...
 localStorage.setItem('gainwright_core_last_test_v1903',JSON.stringify({at:new Date().toISOString(),version:VERSION,results:results}));
}
function mount(){
 var oldModal=$('g19test-modal');if(oldModal)oldModal.remove();
 var oldBtn=$('g19test-btn');if(oldBtn)oldBtn.remove();
 var oldStyle=$('g19test-style');if(oldStyle)oldStyle.remove();
 css();modal();
 ...
 g.GainwrightCoreTestsV1903={version:VERSION,run:runAll,report:report,results:function(){return results.slice()}};
 console.log('[Gainwright Core Tests V19.0.3] ready');
}
