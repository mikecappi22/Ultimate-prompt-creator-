...
assert(!/model\s*[:=][^\n]{0,100}moondream/i.test(src),'Moondream model routing found in V12.1 controller');
assert(!/\/generate[^\n]{0,300}moondream/i.test(src),'Moondream generate routing found');
...
const promoted=JSON.parse(JSON.stringify(baseStructured));promoted.sections[0].confidence='CLEARLY_VISIBLE';
assert(!I.validateStructured(promoted,detailed).ok,'Estimated eye evidence must not be promoted to clear');
const banned=JSON.parse(JSON.stringify(baseStructured));banned.reconstruction_prompt+=' There are no visible blemishes and no artifacts.';
...
console.log('Vision V12.1 RELEASE regression suite PASS');