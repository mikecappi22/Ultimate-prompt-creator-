/* Gainwright Visual Studio V19.0.1 - Core Runtime Test Suite */
(function(g){
'use strict';
if(g.__GAINWRIGHT_CORE_TESTS_V1901__)return;
g.__GAINWRIGHT_CORE_TESTS_V1901__=1;
...
function report(){
 ...
 return ['GAINWRIGHT VISUAL STUDIO V19.0.1 CORE TEST REPORT','Run: '+new Date().toLocaleString(),'Passed: '+p+'/'+results.length+' ('+(results.length?Math.round(p/results.length*100):0)+'%)',''].concat(results.map(function(r){return '['+(r.pass?'PASS':'FAIL')+'] '+r.group+' — '+r.name+(r.msg?' — '+r.msg:'')})).join('\n');
}
function copyReport(){
...
async function runAll(){
 ...
 var tests=[
  ...
  function(){return safe('V18 canonical database','Knowledge',function(){return g.UPCDatabase18&&typeof g.UPCDatabase18.search==='function'?pass('V18 canonical database',(g.UPCDatabase18.records||[]).length+' canonical records loaded','Knowledge'):fail('V18 canonical database','UPCDatabase18 missing','Knowledge')})},
 ...
 localStorage.setItem('gainwright_core_last_test_v1901',JSON.stringify({at:new Date().toISOString(),version:VERSION,results:results}));
}
function mount(){
 css();modal();
 ...
 g.GainwrightCoreTestsV1901={version:VERSION,run:runAll,report:report,results:function(){return results.slice()}};
 console.log('[Gainwright Core Tests V19.0.1] ready');
}
